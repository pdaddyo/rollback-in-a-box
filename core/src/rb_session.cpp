...
bool RollbackSession::sim_has_world() const {
	return sim_ok() && simulation->has_world();
}

int RollbackSession::sim_input_count() const {
	return sim_ok() ? simulation->get_input_count() : 0;
}

void RollbackSession::sim_init_snapshots(int slot_count) {
	if (!sim_ok()) {
		return;
	}
	simulation->init_snapshots(slot_count);
}

bool RollbackSession::sim_save_state(int slot) {
	return sim_ok() && simulation->save_state(slot);
}

bool RollbackSession::sim_load_state(int slot) {
	return sim_ok() && simulation->load_state(slot);
}

uint64_t RollbackSession::sim_state_hash() const {
	return sim_ok() ? simulation->get_state_hash() : 0;
}

void RollbackSession::sim_step_frame(const int64_t *inputs, int count) {
	if (!sim_ok()) {
		return;
	}
	simulation->step_frame(inputs, count);
}

RollbackSession::FrameEntry &RollbackSession::entry_for(int64_t f) {
...
int64_t RollbackSession::min_remote_ack() const {
	int64_t m = INT64_MAX;
	for (int p = 0; p < num_players; ++p) {
		if (p == local_player) {
			continue;
		}
		m = std::min(m, remote_ack[p]);
	}
	return m == INT64_MAX ? -1 : m;
}

double RollbackSession::get_frame_advantage() const {
	double m = 0.0;
	for (int p = 0; p < num_players; ++p) {
		if (p == local_player) {
			continue;
		}
		m = std::max(m, advantage_ema[p]);
	}
	return m;
}

int64_t RollbackSession::min_remote_confirmed() const {
	int64_t m = INT64_MAX;
	for (int p = 0; p < num_players; ++p) {
		if (p == local_player) {
			continue;
		}
		m = std::min(m, confirmed_frame[p]);
	}
	return m == INT64_MAX ? confirmed_frame[local_player] : m;
}

int64_t RollbackSession::safe_frame() const {
	int64_t m = confirmed_frame[local_player];
	for (int p = 0; p < num_players; ++p) {
		m = std::min(m, confirmed_frame[p]);
	}
	int64_t sf = m + 1;
	sf = std::min(sf, current_frame);
	return std::max(sf, (int64_t)0);
}

void RollbackSession::set_simulation(IRollbackSimulation *p_simulation) {
	if (active) {
		return;
	}
...
void RollbackSession::configure(int p_local_player, int p_num_players, int p_input_delay, int p_max_prediction) {
	if (active) {
		return;
	}
	if (p_num_players < 1 || p_num_players > MAX_PLAYERS) {
		return;
	}
	if (p_local_player < 0 || p_local_player >= p_num_players) {
		return;
	}
	if (p_input_delay < 0 || p_input_delay > 16) {
		return;
	}
	if (p_max_prediction < 1 || p_max_prediction > SNAPSHOT_SLOTS - 8) {
		return;
	}
...
void RollbackSession::start() {
	...
	if (sim_input_count() != num_players) {
		return;
	}
	if (active) {
		return;
...
int64_t RollbackSession::get_build_fingerprint() {
	return (int64_t)b3r_determinism_fingerprint();
}

void RollbackSession::sim_one(int64_t f) {
	FrameEntry &e = entry_for(f);
	int64_t inputs[MAX_PLAYERS] = {};
	for (int p = 0; p < num_players; ++p) {
	...
	sim_step_frame(inputs, num_players);
	sim_save_state((int)((f + 1) % SNAPSHOT_SLOTS));
	hashes[(f + 1) % RING] = { f + 1, sim_state_hash() };
}

void RollbackSession::do_rollback() {
	const int64_t target = first_incorrect;
	last_mispredicted_mask = mispredicted_mask;
	if (sim_ok() && simulation->supports_rollback_begin()) {
		simulation->rollback_begin(target, (int)(current_frame - target), (int)mispredicted_mask);
	}
	mispredicted_mask = 0;
...
bool RollbackSession::tick(int64_t local_input) {
	if (!active || !sim_ok()) {
		return false;
	}
...
std::vector<uint8_t> RollbackSession::get_packet() {
	...
	start_f = std::max(start_f, latest - (MAX_SEND_INPUTS - 1));
	start_f = std::max(start_f, (int64_t)0);
	int count = latest >= start_f ? (int)(latest - start_f + 1) : 0;
	out.resize(HEADER_SIZE + 8 * (size_t)count);
	uint8_t *w = out.data();
	write_le<uint32_t>(w, 0, PACKET_MAGIC);
	write_le<uint8_t>(w, 4, PACKET_VERSION);
	write_le<uint8_t>(w, 5, (uint8_t)local_player);
	const int adv = (int)clampv(get_frame_advantage(), -120.0, 120.0);
...
void RollbackSession::ingest_packet(const uint8_t *data, size_t size) {
	const int64_t psize = (int64_t)size;
	if (!active || data == nullptr || psize < (int64_t)HEADER_SIZE) {
		return;
	}
	...
	if (sender_fingerprint != local_fingerprint) {
		if ((incompatible_mask & (1 << sender)) == 0) {
			...
			if (observer != nullptr) {
				observer->on_peer_incompatible(sender, (int64_t)sender_fingerprint);
			}
		}
		return;
	}
	...
	if (psize != (int64_t)(HEADER_SIZE + 8 * (size_t)count)) {
		return;
	}
	...
	remote_current_frame[sender] = std::max(remote_current_frame[sender], their_frame);
	...
	if (ack != NO_FRAME && (int64_t)ack <= latest_local) {
		remote_ack[sender] = std::max(remote_ack[sender], (int64_t)ack);
	}
...
void RollbackSession::check_remote_hash() {
	...
	for (int p = 0; p < num_players; ++p) {
		...
		if (h.hash != remote_hash[p]) {
			...
			if (observer != nullptr) {
				observer->on_desync(remote_hash_frame[p]);
			}
			return;
		}
	}
...
int64_t RollbackSession::get_hash_for_frame(int64_t frame) const {
